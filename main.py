...
AUTO_PROMPT_CACHE_KEY = os.getenv("AUTO_PROMPT_CACHE_KEY", "true").lower() == "true"
...
def convert_response_usage_to_chat_usage(response_usage: Optional[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    if input_details:
        ...
        if "cache_write_tokens" in input_details:
            chat_usage["prompt_tokens_details"]["cache_write_tokens"] = input_details["cache_write_tokens"]
...
def generate_prompt_cache_key(chat_request: ChatCompletionRequest) -> str:
    """为同一会话的增长式请求生成稳定且不泄露内容的缓存路由键。"""
    leading_instructions = []
    first_user_content = None

    for message in chat_request.messages:
        if message.role in ("system", "developer") and first_user_content is None:
            leading_instructions.append({
                "role": message.role,
    ...
    digest = hashlib.sha256(serialized_identity.encode("utf-8")).hexdigest()[:48]
    return f"r2c-{digest}"
...
def convert_chat_to_response_request(chat_request: ChatCompletionRequest) -> Dict[str, Any]:
    ...
    if chat_request.prompt_cache_key:
        response_request["prompt_cache_key"] = chat_request.prompt_cache_key
    elif AUTO_PROMPT_CACHE_KEY:
        response_request["prompt_cache_key"] = generate_prompt_cache_key(chat_request)

    if chat_request.prompt_cache_retention is not None:
        response_request["prompt_cache_retention"] = chat_request.prompt_cache_retention

    if chat_request.prompt_cache_options is not None:
        response_request["prompt_cache_options"] = chat_request.prompt_cache_options
